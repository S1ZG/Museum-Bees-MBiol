...
plot_trends <- function(df, slope_col, xlab) {
  ...
  df <- df %>%
    mutate(
      # Capitalise ecology labels
      ecology = recode(ecology,
        "cavity nester" = "Cavity Nester",
        "excavator" = "Excavator",
        "kleptoparasite" = "Kleptoparasite"
      ),
      
      # Capitalise sex labels
      sex = recode(sex,
        "female" = "Female",
        "male"   = "Male"
      ),
      ...
      species = factor(species, levels = levels(species))
    )
...
plot_trends(year_sex, "year_rescaled.trend", "Estimated coefficient for collection year on log(ITD)") +
...
plot_trends(temp_sex, "mean_preflight_temp.trend", "Estimated coefficient for pre-flight temperature on log(ITD)") +
...
ggplot(coef_allometry, aes(x = estimate, y = species, colour = ecology)) +
  ...
  scale_color_manual(values = c("#fae8b2", "#cb8e73", "#ccb3d6"))
